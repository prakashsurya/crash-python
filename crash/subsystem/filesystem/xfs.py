...
class XFSBufDecoder(Decoder):
    description = "{:x} xfsbuf: offset {}, size {}, block number {}"
    ...
    def __str__(self):
        return self.description.format(int(xfsbuf), xfsbuf['b_file_offset'],
                                       xfsbuf['b_buffer_length'],
                                       xfsbuf['b_bn'])
class XFSBufBioDecoder(Decoder):
    description = "{:x} bio: {} buffer on {}"
...
def item_to_inode_log_item(item):
    """
...
def item_to_dquot_log_item(item):
    """
...
def item_to_quotaoff_log_item(item):
    """
...
def xfs_log_item_typed(item):
    ...
    if li_type == XFS_LI_BUF:
        return types.item_to_buf_log_item(item)
    elif li_type == XFS_LI_INODE:
        return types.item_to_inode_log_item(item)
    elif li_type == XFS_LI_EFI:
        return types.item_to_efi_log_item(item)
    elif li_type == XFS_LI_EFD:
        return types.item_to_efd_log_item(item)
    elif li_type == XFS_LI_IUNLINK:
        # There isn't actually any type information for this
        return li_type
    elif li_type == XFS_LI_DQUOT:
        return types.item_to_dquot_log_item(item)
    elif li_type == XFS_LI_QUOTAOFF:
        return types.item_to_quotaoff_log_item(item)
    raise RuntimeError("Unknown AIL item type {:x}".format(li_type))

def xfs_format_xfsbuf(buf):
    state = ""
