...
TEMP=$(getopt -o 'vr:d:m:D:b:h' --long 'verbose,root:,modules-debuginfo:,modules:,vmlinux-debuginfo:,build-dir:,gdb,valgrind,help' -n "$(basename $0)" -- "$@")
...
s = "$MODULES_DEBUGINFO"
if len(s) > 0:
    modules_debuginfo = s.split(" ")

try:
    x = crash.session.Session(roots=roots, vmlinux_debuginfo=vmlinux_debuginfo,
                              modules=modules,
                              modules_debuginfo=modules_debuginfo,
